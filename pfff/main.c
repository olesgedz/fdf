...
static void	draw_line(t_point3d f, t_point3d s)
{
	t_point3d	delta;
	t_point3d	sign;
	t_point3d	cur;
	...
	{
		//iso(&cur);
		ft_img_pixel_put(cur.x_iso  * map.scale + data.w_width/2 - map.width , cur.y_iso * map.scale + data.w_height/2 - map.height, 0xff0000);
...
void ft_draw(t_point3d *point)
{
	...
	if (point->z != 0)
	 color = 0xff0000;
	ft_img_pixel_put(point->x_iso * map.scale + data.w_width/2 - map.width, point->y_iso * map.scale + data.w_height/2 - map.height,  color);
...
{	// void new = mlx_new_image(data.mlx_ptr, 500 , 500);
	...
	i = 0;
		...
		while (i < map.width * map.height)
		{
			...
			draw_line(map.points[i], map.points[i + 1]);
			i++;
		}
